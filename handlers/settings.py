...
# ==================== POSTLARNI TOZALASH (1-bosqich: So'rash) ====================
...
async def confirm_clear_posts(call: CallbackQuery):
    ...
    await call.message.edit_text("⚠️ **DIQQAT!**\n\nBu amal sizning navbatda turgan **BARCHA** postlaringizni o'chirib yuboradi.\n\nAniq o'chirilsinmi?", reply_markup=kb)

# ==================== POSTLARNI TOZALASH (2-bosqich: O'chirish) ====================
@settings_router.callback_query(F.data == "set_clear_posts_confirm")
async def clear_posts_confirmed(call: CallbackQuery):
    uid = call.from_user.id
    
    # Kutilayotgan postlarni bazadan olamiz
    pending_posts = await db.db.posts.find({"owner_id": uid, "status": "pending"}).to_list(length=None)
    
    # Navbat kanalidan xabarlarni o'chiramiz
    ...
    # Endi bazadan o'chiramiz
    await db.db.posts.delete_many({"owner_id": uid, "status": "pending"})
    
    await call.message.edit_text("✅ Barcha navbatdagi postlar o'chirildi!",
